...
class DataNormalize:
    '''Fast normalize. by Andrease
    '''
    def __init__(self, mode, mean, std, num_channel=3):
        self.mode = mode
        self.mean = mean
        self.std = std
    ...
    def __call__(self, data, out=None):
        ...
        for kk in range(nframes):
            ...
            out[self.num_channel*kk:self.num_channel*(kk+1), :, :] = (
                data[kk].transpose(2, 0, 1)) / max_value
        
        # normalize by chabbels
        for i in range(self.num_channel):
            out[i::self.num_channel] -= self.mean[i]
            out[i::self.num_channel] /= self.std[i]
        return out
