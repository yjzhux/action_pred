...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, in_channel=3, num_classes=1000, fix=False):
        self.inplanes = 64
...
def change_input_layer(old_params, in_channel):
    '''change key names in the model'''
    ...
    for layer_key in allKeyList:
        if layer_count >= len(allKeyList)-2:
            ...
            if layer_count == 0:
                ...
                flow_weight = rgb_weight_mean.unsqueeze(1).repeat(1,in_channel,1,1)
...
def resnet18(pretrained=False, in_channel=3, **kwargs):
    ...
    if pretrained:
        ...
        if in_channel != 3:     # for image
            new_pretrained_dict = change_input_layer(pretrained_dict, in_channel)
        else:                    # for flow or pose
            new_pretrained_dict = pretrained_dict
